    ...
    $.widget('ech.multiselect', {
        ...
        _create: function() {
            ...
            var $footer = ( $( document.createElement('div') ) )
                .addClass('ui-footer')
                .append( this._linkHTML('<a class="{{class}} multi-select-close" title="{{title}}">취소</a>', 'close'),this._linkHTML('<a class="multi-select-submit" title="submit">적용</a>', 'close')  );
            ...
            var $menu = ( this.$menu = $( document.createElement('div') ) )
                .addClass('ui-multiselect-menu ui-widget ui-widget-content ui-corner-all'
                    ...
                    + (this.options.classes ? ' ' + this.options.classes : ''))
                .append($header, $checkboxes, $footer);
        ...
        update: function(isDefault) {
            ...
            if (numChecked) {
                if (typeof selectedText === 'function') {
                    ...
                    var firstTitle = '';
                    if($checked.length > 0){
                        $checked.map(function(index, element) {
                            if(index == 0){
                                firstTitle = $(this).next().text().replace(/\n$/, '');
                            }
                        });
                    }
                    if(numChecked < 2){
                        ...
                        value = firstTitle + ' 외 ' + (numChecked-1);
                    }
