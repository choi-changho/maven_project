(function($) {
	...
	var default_options = {
		...
		aggregations : ['-', 'weekly', 'monthly', 'daily'],
		values : {}

	};

	var default_aggregation = 'weekly';	//daily
	...
	var internal = {
		...
		createElements : function($target) {
			...
			$target.html(
				'<div class="date-range-field">'+
					...
					//'<a href="#">'+
						'<span class="select_name">기간:</span>'+
						'<span class="select_name_sub">'+
							'<span class="main">2021. 01. 04 ~ 2021. 01. 10</span>'+
							'<span>비교: <i class="comparison">2020. 12. 18 ~ 2021. 01. 03</i></span>'+
						'</span>'+
			...
			if (!$dropdown) {
				...
				'<div id="datepicker-dropdown">'+
			   	   ...
			   	   */
						'<div class="main-daterange">'+
							...
							'<div class="date__dr">'+
								...
								//'<span class="dr dr1 from" lastSel="0" /></span> - <span class="dr dr1 to" lastSel="1" /></span>'+
						...
						'<div class="comparison-daterange">'+
							'<div class="date__title"><input type="checkbox" checked="checked" class="enable-comparison" /> 기간 비교 사용</div>'+
							'<div>'+
								'<select class="comparison-preset">'+
									'<option value="previousperiod" selected="selected">이전 기간</option>'+
									'<option value="previousyear">이전 연도</option>'+
					...
					'</div>'+
					'<div class="btn-group">'+
						'<button class="btn btn-mini" id="button-cancel">취소</button>'+
						'<button class="btn btn-mini" id="button-ok">확인</button>'+
					'</div>'+
				'</div>');
				...
				$datepicker.DatePicker({
					...
					calendars: 1,
					inline: true,
					//date: [new Date('2021-03-02'), new Date('2021-03-05'), new Date('2020-09-10'), new Date('2020-09-16')],
					//date: ['2021-03-02', '2021-03-05', '2021-03-06', '2021-03-09'],
		...
		updateDateField : function($target) {
			...
			if (values.comparisonEnabled && values.dr2from && values.dr2to) {
				$('.comparison', $target).text(values.dr2from + ' - ' + values.dr2to);
				//$('.comparison', $target).show();
				$('span.comparison-divider', $target).show();
			} else {
				$('span.comparison-divider', $target).hide();
				//$('.comparison', $target).hide();
			}
			return true;
		},
		...
		populateDateRangePresets : function() {
			...
			//
			$('.date-ranges-form select').niceSelect();	//selectbox design
		},
