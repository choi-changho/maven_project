    ...
    _handler: function(e) {
      ...
      if (term) {
         this._trigger('filter', e, filteredInputs);
          // console.log(this.$rows.prevObject);
      }
      if (!this.instance.options.listbox && this.instance._isOpen) {
         this.instance._setMenuHeight(false);
	  ...
	  var isAllTrue = true;
		...
		$.each(this.$rows, function(index, value) {
			if(!$(value).hasClass(hiddenClass)) {
                //console.log(!$(value).hasClass(hiddenClass));
				isAllTrue = false;
			}
		});
		if(isAllTrue) {
            $(this).get(0).$rows.prevObject.append('<li class="empty-text">검색 결과가 없습니다.</li>');
		} else{
            $(this).get(0).$rows.prevObject.find('.empty-text').remove();
        }
      return;
    },
